...
# FILE SETTINGS
# =========================
FILE = Path(r"C:\Users\gigih\OneDrive\School\Week4\InsuranceData.xlsx")
...
def main():
    ...
    for i in range(len(score)):
        row = score.iloc[i]
        pred, conf, path = score_one(row, rules)
        preds.append(pred)
    ...
    # Save scored file (optional but useful)
    out_file = FILE.parent / "NewApplicants_with_predictions.csv"
    ...
    # Q3
    # 47yo man, CompClaims=No, LatePayments=No, AtFault=0, Claims=1
    ...
    # Q4
    # For the described person:
    # AtFault=2 => At Fault Accidents > 1.5
    # Age=55 => Age ≤ 59.5 (within AtFault>1.5 branch)
    # Moving Violations="Yes" (several)
    # AtFault ≤ 2.5 (since 2)
    # Age > 17.5 (55)
    # Next split that decides between Potentially High Risk vs High Risk - Do Not Insure should be Payment Method.
    q4_partial = [
        ("At Fault Accidents", ">", "0.500"),
        ("Age", "≤", "65.500"),
        ("Age", "≤", "64.500"),
    ...
    # Q7
    # Among predicted High Risk - Do Not Insure, highest confidence(High Risk - Do Not Insure)
    ...
    # Q9
    # If predicted Potentially High Risk with 63.4% confidence, what's confidence it may be High Risk - Do Not Insure?
    # Find rows matching that confidence (within tolerance), then read confidence(High Risk - Do Not Insure).
    ph_mask = scored["prediction(Insurance Category)"] == "Potentially High Risk"
    tol = 0.0005
    target_conf = 0.634
    candidates = scored.loc[ph_mask & (scored["confidence(Potentially High Risk)"].sub(target_conf).abs() <= tol)]
    if len(candidates) == 0:
        # If rounding differs (RapidMiner displays 1 decimal), loosen tolerance a bit
        tol2 = 0.005
        candidates = scored.loc[ph_mask & (scored["confidence(Potentially High Risk)"].sub(target_conf).abs() <= tol2)]

    if len(candidates) == 0:
        print("Q9) No row found with confidence(Potentially High Risk) ≈ 63.4%.")
    else:
        # If multiple, take the first (they should have same pair if it's the textbook case)
        ...
        print(f"Q9) When Potentially High Risk confidence is {ph_conf:.1f}%, High Risk - Do Not Insure confidence is {hr_conf:.1f}%")
